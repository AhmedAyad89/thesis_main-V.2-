...
% C-RNN(A) refers to a network trained with KL annealing, C-RNN(W) for networks trained with the compression loss weighting (cf. \cref{sec:model}), and C-RNN(A+W) to our full model. We will refer to our baseline model based on MC dropout~\cite{gal2016dropout} as the MC-dropout model. 
